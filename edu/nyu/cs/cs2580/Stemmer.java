...
class Stemmer {
	...
	public static String stemAWord(String wordToBeStemmed) {
		Stemmer s = new Stemmer();
		char[] arr = wordToBeStemmed.toCharArray();
		char w[] = new char[501];
		int i = 0;
		StringBuffer finalFile = new StringBuffer();
		boolean flag = false;
		while (true) {
			if (i >= arr.length) {
				flag = true;
			...
			if (Character.isLetter((char) ch)) {
				...
				while (true) {
					...
					if (!Character.isLetter((char) ch) || flag) {
						for (int c = 0; c < j; c++) {
							s.add(w[c]);
						}
						s.stem();
						{
							String u;
							u = s.toString();
							finalFile.append(new StringBuffer(u));
						}
						break;
					}
				}
