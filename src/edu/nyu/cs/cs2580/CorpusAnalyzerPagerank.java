...
public class CorpusAnalyzerPagerank extends CorpusAnalyzer {
	...
	private HashMap<Integer, Double> I = new HashMap<Integer, Double>();
	int index;
	int noOfFiles;
	...
	public void compute() throws IOException {
		...
		while (i < index) {
			double probability_P = (1 / noOfPages);
		...
		while (iterations < 2) {
			...
			double random_selection = lambda / noOfPages;
			...
			while (nums >= noOfFiles / 10) {
				...
				for (int docid : graph.keySet()) {
					...
					if (Q.size() > 0) {
						for (int page : Q) {
							double Rq = R.get(page);
							double Ip = I.get(docid);
						...
						for (int page : I.keySet()) {
							double Rq = R.get(page);
							double Ip = I.get(docid);
							double Rp = Rq + (((1 - lambda) * Ip) / I.size());
							R.put(page, Rp);
						}
					}
					I = R;
	...
	private void serializeR() throws IOException {
		...
		for (int docid : R.keySet()) {
			aoos.write(docid + "\t");
			double pageRankValue = R.get(docid);
