...
public class DecompressionUtility {
	...
	public static Vector<Integer> decodeByteAlign(Vector<Character> encodedList) {
		...
		while ((i + 1) < (encodedList.size())) {
			...
			while (!endingBlock && (pos + 1) < (encodedList.size())) {
				binaryBlock1 = convertHexToBinaryBlockOf4(encodedList.get(pos));
				binaryBlock2 = convertHexToBinaryBlockOf4(encodedList.get(pos + 1));
	...
	public static String convertHexToBinaryBlockOf4(char hexChar) {
		if (hexChar == '0')
			return "0000";
		else if (hexChar == '1')
			return "0001";
	...
	public static char convertBinaryBlockOf4ToHex(String input) {
		int sum = 0;
