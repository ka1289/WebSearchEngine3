...
/**
 * This is the main entry class for the Search Engine. Usage (must be running
 * from hwX/gXX/ directory): 0) Compiling javac src/edu/nyu/cs/cs2580/*.java 1)
 * Mining java -cp src edu.nyu.cs.cs2580.SearchEngine \ --mode=mining
 * --options=conf/engine.conf 2) Indexing java -cp src
 * edu.nyu.cs.cs2580.SearchEngine \ --mode=index --options=conf/engine.conf 3)
 * Serving
 * 
 * @CS2580: you must use a port number 258XX, where XX is your group number.
 *          java -cp src -Xmx256m edu.nyu.cs.cs2580.SearchEngine \ --mode=serve
 *          --port=[port] --options=conf/engine.conf You must ensure your
 *          program runs with maximum heap memory size -Xmx512m
 * 
...
public class SearchEngine {
	...
	private static void parseCommandLine(String[] args) throws IOException, NumberFormatException {
		...
		Check(MODE != Mode.SERVE || PORT != -1, "Must provide a valid port number (258XX) in serve mode!");
